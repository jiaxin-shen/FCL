...
def train(args, model, device, loader, rec_loss, optimizer, epoch):
    ...
    for batch_idx, (data, label) in enumerate(loader):
        ...
        loss_pred = F.nll_loss(F.log_softmax(pred, dim=1), label)  #通常说的损失  应该就是cls
        acc = cal_acc(pred, label)
        reconstruction_loss = rec_loss(out.view(data.size(0), 1, 28, 28), data) # l rec
        quantity_loss = quantization_loss(cpt)  #l qua
        batch_dis_loss = batch_cpt_discriminate(update, att)# l dis
        consistence_loss = att_consistence(update, att) # l con
